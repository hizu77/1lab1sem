...
void PrintNFirstString(uint64_t n, const char *name, char d) {
    std::ifstream myFile(name);
    char symbol;
    char temp;
    bool flag = true;
    uint64_t num_of_str = 1;
...
void PrintNLastString(uint64_t n, const char *name, char d) {
    std::ifstream myFile(name);
    char symbol;
    uint64_t num_of_str = 1;
    while (myFile.get(symbol)) {
...
struct Args {
    ...
    bool correct{true};
    uint64_t line = std::numeric_limits<uint64_t>::max();
};
